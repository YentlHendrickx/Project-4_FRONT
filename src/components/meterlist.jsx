...
function MeterList() {
  ...
  useEffect(() => {
    const fetchUserData = async () => {
      ...
      const result = await axios.get(
        process.env.REACT_APP_API_URL + "User/" + userData.userId
      );
  ...
  async function handleEdit(event) {
    ...
    const meterResponse = await axios.get(
      process.env.REACT_APP_API_URL + "Meter"
    );

    const foundMeter = meterResponse.data.find(
      (meter) =>
        meter.meterDeviceId === formData.meterId && meter.rpId === formData.rpId
    );
    ...
    if (foundMeter === undefined) {
      ...
      const resp = await axios.put(
        process.env.REACT_APP_API_URL + "UserMeter/" + formData.id,
        userMeterDto
      );
      ...
      if (resp.status === 204) {
        ...
        const meterIndex = metersList.findIndex(
          (meter) =>
            meter.meterId === formData.meterId && meter.rpId === formData.rpId
        );
        if (meterIndex !== -1) {
          // Uppdate meter
  ...
  async function handleDelete(index) {
    ...
    await axios.delete(
      process.env.REACT_APP_API_URL + "UserMeter/" + removalId
    );
  ...
  async function handleCreate(event) {
    ...
    const foundMeter = meterResponse.data.find(
      (meter) =>
        meter.meterDeviceId === newData.meterId && meter.rpId === newData.rpId
    );
    ...
    if (foundMeter === undefined) {
      ...
      var meterInList = metersList.find(
        (meter) =>
          meter.rpId === newData.rpId && meter.meterDeviceId === newData.meterId
      );
      ...
      if (meterInList === undefined) {
        ...
        await axios
          .post(process.env.REACT_APP_API_URL + "UserMeter", userMeterDto)
          .then((res) => {
            const newMetersList = [...metersList];
    ...
    <div className="w-full h-screen overflow-auto">
      <form onSubmit={handleEdit}>
        <table className="border-collapse w-full">
          <thead className="border-b bg-gray-600">
            <tr className="w-full">
              <th className="text-md font-bold text-white px-6 py-6 text-left w-1/2">
                <div className="flex">
                  Raspberry ID
                  <div className="scale-[0.7] -mt-2 ml-1 cursor-pointer ">
                    <span
                      style={{ marginTop: "-100px" }}
                      title="Fill in the code that was provided to you (sticker)"
                    >
                      {
                        icons[icons.findIndex((i) => i.name === "InfoOutlined")]
                          .icon
                      }
                    </span>
                  </div>
              ...
              <th className="text-md font-bold text-white px-6 py-6 text-left w-[30%]">
                <div className="flex ">
                  ...
                  <div className="scale-[0.7] -mt-2 ml-1 cursor-pointer ">
                    <span
                      style={{ marginTop: "-100px" }}
                      title="Fill in the id of the digital meter where you plugged the raspberry pi in. You can find this on the front of your meter. Example: 1SAGXXXXXXXXXX"
                    >
                      {
                        icons[icons.findIndex((i) => i.name === "InfoOutlined")]
                          .icon
              ...
              <th className="text-md font-bold text-white px-6 py-6 text-left w-[10%]">
                Actions
              </th>
            </tr>
          </thead>
                  ...
                  <>
                    ...
                    <td className="text-md text-gray-900 font-light px-6 py-4 w-[10%]">
                      <div className="flex gap-4">
                        ...
                        >
                          {
                            icons[icons.findIndex((i) => i.name === "cancel")]
                              .icon
                          }
                        </button>
                        <button
                          className="text-save hover:text-saveHover"
                          type="submit"
                        >
                          {
                            icons[icons.findIndex((i) => i.name === "save")]
                              .icon
                          }
                        </button>
                      </div>
                  ...
                  <>
                    ...
                    <td className="text-md text-gray-900 font-light px-6 py-4 w-[10%]">
                      <div className="flex gap-4">
                        ...
                        >
                          {
                            icons[icons.findIndex((i) => i.name === "edit")]
                              .icon
                          }
                        </button>
                        <button
                        ...
                        >
                          {
                            icons[icons.findIndex((i) => i.name === "delete")]
                              .icon
                          }
                        </button>
                      </div>
      ...
      {!creating && !editing && (
        <div className="flex flex-col w-full mt-2">
          <button
            className="bg-create text-uiLight rounded-md p-2 self-end mr-4"
      ...
      {creating && (
        <form className="mt-5 flex flex-col" onSubmit={handleCreate}>
          ...
          <input
            className="mx-2 text-sm p-2 mb-1"
            type="text"
            name="address"
            placeholder="Address"
          ...
          <div className="self-end mt-2 mr-4 flex gap-2">
            <button
              className="bg-delete text-uiLight rounded-md w-[4rem] h-[2.5rem]"
              type="button"
            ...
            <button
              className="bg-save text-uiLight rounded-md w-[4rem] h-[2.5rem]"
              type="submit"
            >
              Create
            </button>
